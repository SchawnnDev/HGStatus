...
public class ServerInfo {
    ...
    public ServerStatus getServerStatus(){
        ...
        if(motd.contains("Hunger Games en cours") || motd.contains("é7Hunger Games en cours"))
            return ServerStatus.GAME;
        else if(motd.contains("Génération en cours") || motd.equals("é7Génération en cours"))
            return ServerStatus.GENERATING;
        else if(motd.contains("Hunger Games debute") || motd.equals("é2Hunger Games debute"))
    ...
    public int getData(){
        ...
        if(serverStatus == ServerStatus.GENERATING) {
            String motd = _motd.replaceAll("Génération", "").replaceAll("é7", "").replaceAll("en", "").replaceAll("cours", "").replace('(', ' ').replace(')', ' ').replaceAll("%", "").replaceAll(" ", "");
